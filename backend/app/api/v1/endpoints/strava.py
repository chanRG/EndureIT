...
def get_strava_activities(
    ...
    after: Optional[int] = Query(None, description="Unix timestamp to get activities after"),
    before: Optional[int] = Query(None, description="Unix timestamp to get activities before"),
    ...
    try:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                logger.warning(f"Rate limited, using cached data for user {current_user.username}")
            else:
                raise
        ...
        logger.info(f"Returned {len(paginated)} cached activities for user {current_user.username}")
        return paginated
...
def get_strava_activity(
    ...
    try:
        ...
        activity_data = {
            ...
            } if activity.map_polyline else None,
    ...
    try:
        ...
        try:
            ...
            if sync_result.get('synced', False):
                ...
                efforts_synced = sync_service.sync_best_efforts(limit=None)
                if efforts_synced > 0:
                    logger.info(f"Synced missing best efforts for {efforts_synced} existing activities")
        except StravaAPIError as e:
...
def get_pr_history(
    ...
    try:
        ...
        ).order_by(StravaActivity.start_date.asc()).all()  # Oldest first for chronological order
...
def get_heart_rate_zones(
    max_hr_override: Optional[int] = Query(None, ge=80, le=240, description="Override maximum heart rate in bpm"),
    current_user: User = Depends(deps.get_current_user),
    db: Session = Depends(get_db)
) -> Any:
    """
    ...
    try:
        ...
        return analysis_service.get_last_month_zone_analysis(max_hr_override=max_hr_override)
    except StravaAPIError as e:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail=str(e)
