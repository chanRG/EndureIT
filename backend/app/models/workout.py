...
class Workout(Base):
    ...
    workout_type: Mapped[WorkoutType] = mapped_column(
        Enum(WorkoutType, native_enum=False), nullable=False, index=True
    )
    ...
    average_pace: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # min/km
    elevation_gain: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # meters

    # Intensity & Effort
    intensity: Mapped[Optional[IntensityLevel]] = mapped_column(
        Enum(IntensityLevel, native_enum=False), nullable=True
    )
