...
class ProgressService:
    ...
    def get_progress_entries(
        ...
        return query.order_by(desc(ProgressEntry.entry_date)).offset(skip).limit(limit).all()
    ...
    def get_latest_weight(db: Session, user_id: int) -> Optional[float]:
        ...
        ).order_by(desc(ProgressEntry.entry_date)).first()
    ...
    def get_weight_change(
        ...
        oldest = db.query(ProgressEntry).filter(
            ProgressEntry.user_id == user_id,
            ProgressEntry.weight_kg.isnot(None),
            ProgressEntry.entry_date >= since_date
        ).order_by(ProgressEntry.entry_date).first()
        
        # Get latest entry
        latest = db.query(ProgressEntry).filter(
            ProgressEntry.user_id == user_id,
            ProgressEntry.weight_kg.isnot(None)
        ).order_by(desc(ProgressEntry.entry_date)).first()
