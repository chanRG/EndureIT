...
class GoalService:
    ...
    def create_goal(db: Session, user: User, goal_data: GoalCreate) -> Goal:
        ...
        goal = Goal(user_id=user.id, **goal_data.model_dump())
        db.add(goal)
        db.commit()
        db.refresh(goal)
    ...
    @staticmethod
    def update_goal(
        db: Session, goal_id: int, user_id: int, goal_data: GoalUpdate
    ) -> Optional[Goal]:
        """Update a goal.
    ...
    def update_goal_progress(
        db: Session, goal_id: int, user_id: int, current_value: float
    ) -> Optional[Goal]:
