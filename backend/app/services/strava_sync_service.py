...
class StravaSyncService:
    ...
    def __init__(self, db: Session, user: User):
        ...
        def save_refreshed_tokens(access_token: str, refresh_token: str, expires_at: int):
    ...
    def sync_activities(self, force_full_sync: bool = False) -> Dict[str, Any]:
        ...
        last_activity = self.db.query(StravaActivity).filter(
        ...
        if not force_full_sync and last_activity and last_activity.last_synced:
            ...
            if time_since_sync < timedelta(hours=1):
                logger.info(f"Using cached data for user {self.user.id}, last synced {time_since_sync.seconds // 60} minutes ago")
                return {
                    "synced": False,
        ...
        try:
            ...
            return {
                ...
                "total_cached": self.db.query(StravaActivity).filter(
                    StravaActivity.user_id == self.user.id
                ).count()
            }
    ...
    def sync_best_efforts(self, limit: int = None) -> int:
        ...
        query = self.db.query(StravaActivity).filter(
            StravaActivity.user_id == self.user.id,
            StravaActivity.activity_type.ilike('%run%'),  # All runs, not just those with achievements
        ...
        logger.info(f"Fetching best_efforts for {len(activities_needing_efforts)} running activities")
        ...
        for activity in activities_needing_efforts:
            try:
                ...
                if activity.best_efforts:
                    logger.debug(f"Cached {len(activity.best_efforts)} best_efforts for activity {activity.strava_id}")
                else:
                    logger.debug(f"No best_efforts for activity {activity.strava_id} (cached empty list)")
                    
            except Exception as e:
                logger.warning(f"Failed to fetch best efforts for activity {activity.strava_id}: {str(e)}")
                continue
    ...
    def _create_activity(self, data: Dict[str, Any]) -> StravaActivity:
        ...
        map_polyline = map_data.get('summary_polyline') if isinstance(map_data, dict) else None
    ...
    def get_cached_activities(self) -> List[StravaActivity]:
        ...
        return self.db.query(StravaActivity).filter(
            StravaActivity.user_id == self.user.id
            ...
            if activity.best_efforts:
                for effort in activity.best_efforts:
                    ...
                    if name:
                        ...
                        if name not in all_best_efforts or effort.get('elapsed_time', 999999) < all_best_efforts[name].get('elapsed_time', 999999):
