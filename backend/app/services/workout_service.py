...
class WorkoutService:
    ...
    def create_workout(db: Session, user: User, workout_data: WorkoutCreate) -> Workout:
        ...
        logger.info(f"Created workout {workout.id} with {len(workout_data.exercises)} exercises")
        return workout
    ...
    def get_workout_stats(
        ...
        most_common = max(workouts_by_type.items(), key=lambda x: x[1])[0] if workouts_by_type else None
        ...
        return WorkoutStats(
            ...
            average_duration_minutes=(total_duration / 60 / total_workouts) if total_workouts else 0,
