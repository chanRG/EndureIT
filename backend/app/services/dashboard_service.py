...
class DashboardService:
    ...
    def get_user_dashboard(db: Session, user_id: int) -> UserDashboard:
        ...
        # Workouts this week
        workouts_this_week = db.query(func.count(Workout.id)).filter(
            Workout.user_id == user_id,
            Workout.is_completed == True,
        ...
        # Workouts this month
        workouts_this_month = db.query(func.count(Workout.id)).filter(
            Workout.user_id == user_id,
            Workout.is_completed == True,
        ...
        recent_workouts = [WorkoutListResponse.model_validate(w) for w in recent_workouts_data]
