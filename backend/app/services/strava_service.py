...
class StravaService:
    ...
    def __init__(self, access_token: Optional[str] = None, refresh_token: Optional[str] = None, on_token_refresh=None):
        """
            ...
            if response.status_code == 401:
                ...
                if retry_on_401 and self.refresh_token:
                    try:
                        ...
                        return self._make_request(method, endpoint, params, data, retry_on_401=False)
                    except StravaAPIError as e:
                        logger.error(f"Token refresh failed: {str(e)}")
                        raise StravaAPIError("Access token expired and refresh failed. Please re-authenticate.")
                else:
                    raise StravaAPIError("Access token expired or invalid. Please re-authenticate.")
            
            # Check for forbidden (scope issues)
            if response.status_code == 403:
                logger.error(f"Strava API forbidden error: {response.text}")
                raise StravaAPIError("Access forbidden. Please ensure your Strava app has the required scopes (activity:read_all).")
        ...
        try:
            ...
            if self.on_token_refresh:
                try:
                    ...
                    logger.error(f"Failed to persist refreshed tokens via callback: {str(e)}")
...
def create_strava_service(
    ...
    """
    return StravaService(access_token=access_token, refresh_token=refresh_token, on_token_refresh=on_token_refresh)
